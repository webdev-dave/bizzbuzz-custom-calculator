...
import { selectAdditionalDataCollapsedState } from "./collapseButton/collapseSlice";

const AdditionalData = () => {
  const quantitiesArr = useSelector(selectQuantities);
  const additionalDataIsCollapsed = useSelector(
    selectAdditionalDataCollapsedState
  );
  ...
  return (
    <div id="additional-data-container" className="additional-data">
      ...
      <CollapseButton />
      {/* only render additional-data-grid if additionalDataIsCollapsed === false (i.e. when additional data section is expanded) */}
      {!additionalDataIsCollapsed &&
        ...
        //additional data conditional closer closer
      }
    </div>
  );
};
